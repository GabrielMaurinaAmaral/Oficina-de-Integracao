...
#define SAMPLE_DEVICE_NAME "ESP_SPP_SERVER_2" // Define o nome do bluetooth do esp
...
volatile unsigned long recebidoData = 0;
volatile int recebidoBitLength = 0;
volatile int recebidoProtocol = 0;
...
void init_ble_spp(void)
{
    ...
    while (1)
    {
        ...
        if (global_string[0] != 0 && (strstr(global_string, "gravar") != NULL))
        {
            ...
            recebidoData = getReceivedValue();
            recebidoBitLength = getReceivedBitlength();
            recebidoProtocol = getReceivedProtocol();

            snprintf(global_string, sizeof(global_string), "%lu;%d;%d", recebidoData, recebidoBitLength, recebidoProtocol);
        ...
        else if (global_string[0] != 0 && (strstr(global_string, "transmitir") != NULL))
        {
            ...
            if (recebidoProtocol >= 0 && recebidoProtocol < sizeof(proto) / sizeof(proto[0]))
            {
                const Protocol *Protocol = &proto[recebidoProtocol];

                uint32_t start_time = xTaskGetTickCount();
                uint32_t duration = pdMS_TO_TICKS(10000); 
        
                while ((xTaskGetTickCount() - start_time) < duration)
                {
                    sendRFSignal(recebidoData, recebidoBitLength, Protocol);
                    
                }
            }
            else
            {
                ESP_LOGE(GATTS_TABLE_TAG, "Invalid parameters for sendRFSignal: data=%lu, bitLength=%d, protocol=%d", recebidoData, recebidoBitLength, recebidoProtocol);
            }
        ...
        else if (global_string[0] != 0)
        {
            ...
            if (sscanf(global_string, "%lu;%d;%d", &tempData, &tempBitLength, &tempProtocol) == 3)
            {
                recebidoData = tempData;
                recebidoBitLength = tempBitLength;
                recebidoProtocol = tempProtocol - 1;
                ESP_LOGI(GATTS_TABLE_TAG, "\nDados recebidos: %lu, BitLength: %d, ProtocolIndex: %d", recebidoData, recebidoBitLength, recebidoProtocol);
            }
            else
                ESP_LOGE(GATTS_TABLE_TAG, "Erro ao analisar global_string: %s", global_string);
